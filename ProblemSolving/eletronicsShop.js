...
function getMoneySpent(keyboards, drives, b) {
    ...
    for(let i = 0; i < keyboards.length; i++){
        for(let j = 0; j <= drives.length; j++){
            if(keyboards[i] <= b && drives[j] <= b && keyboards[i] + drives[j] <= b){
                maxValue.push(keyboards[i] + drives[j])
            }else if(keyboards[i] > b || drives[j] > b || keyboards[i] + drives[j] > b)
                answer = -1
        }
    }

    let result = maxValue.sort((a, b) => a - b)
    result = maxValue[maxValue.length - 1]
